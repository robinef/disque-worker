 ...
 * @param int maxlen
 */
Worker.prototype.addJob = function(queue, payload, timeout, maxlen) {
  var _this = this;
  var options = [queue, payload, parseInt(timeout)];
  if (typeof maxlen !== 'undefined' && maxlen !== null) {
    options.push('MAXLEN');
    options.push(parseInt(maxlen));
  }
  return new Promise(function(resolve, reject) {
    _this.client.send_command("ADDJOB", options, function(err, res) {
      if (err !== null) {
        return (reject(err));
      }
      return (resolve(res));
